...
/* ============================================================ */
/* 3. 仅当连续输入 ￥￥ 时：变为 $$$$ 且光标在 $$|$$ 中间；单独 ￥ 不处理   */
/*    用 input 事件检测，兼容 IME 输入法                            */
/* ============================================================ */
(function () {
  if (window.__roamYenToDollarInited) return;
  window.__roamYenToDollarInited = true;
  ...
  function isYen(ch) {
    return ch === YEN_FULL || ch === YEN_HALF;
  }

  function twoCharsBeforeCursorAreYenYen(text, start) {
    if (!text || start < 2) return false;
    var a = text[start - 2];
    var b = text[start - 1];
    return isYen(a) && isYen(b);
  }

  function getRoamTextarea(el) {
    var node = el && el.nodeType === 1 ? el : el && el.target;
    if (!node || node.tagName !== "TEXTAREA") return null;
    if (!node.closest(".roam-block") && !node.closest(".rm-block__input")) return null;
    if (node.closest(".rm-code-block")) return null;
    return node;
  }

  function fixDollarYenDollar(ta) {
    var text = ta.value;
    var start = ta.selectionStart;
    var idx = text.indexOf("$$");
    if (idx === -1) return false;
    var afterDd = text.slice(idx + 2);
    var yenAt = -1;
    if (isYen(afterDd[0])) yenAt = idx + 2;
    if (yenAt === -1) return false;
    if (text.slice(yenAt + 1, yenAt + 3) !== "$$") return false;
    ta.value = text.slice(0, yenAt) + text.slice(yenAt + 1);
    ta.selectionStart = ta.selectionEnd = yenAt;
    ta.dispatchEvent(new Event("input", { bubbles: true }));
    return true;
  }

  document.addEventListener("input", function (e) {
    var ta = getRoamTextarea(e);
    if (!ta) return;
    var text = ta.value;
    var start = ta.selectionStart;
    var end = ta.selectionEnd;
    if (twoCharsBeforeCursorAreYenYen(text, start)) {
      e.preventDefault();
      e.stopImmediatePropagation();
      var before = text.slice(0, start - 2);
      var after = text.slice(end);
      ta.value = before + DOLLAR_FOUR + after;
    ...
    setTimeout(function () {
      ...
      if (fixDollarYenDollar(ta)) return;
    }, 0);
  }, true);
})();
