...
- **已定义的字体栈不得修改、替换或重定义**：包括 `font-family` 字体变量（如 `--body-font`、`--heading-font`、`--code-font` 等）及其值，以及具体选择器上已写死的 `font-family`。
...
- **已定义的色值不得修改、替换或重定义**：包括既有配色变量（如 `--text`、`--bg`、`--primary`、`--m-*` 等）及其值，以及具体选择器上已写死的颜色。
