...
(function() {
    ...
    const addStyles = () => {
        ...
        style.textContent = `
            ...
            .tdd-divider { flex: 0 0 1px; background-color: rgb(57, 75, 89); opacity: 0.3; }
            .tdd-title { display: flex; align-items: center; font-size: 14px; opacity: 80%; letter-spacing: 0.04em; margin-bottom: 8px; font-weight: 600; }
            .tdd-title .bp3-icon { margin-right: 8px; opacity: 0.7; }
            .tdd-buttons { display: flex; flex-direction: column; gap: 6px; }
            .tdd-button { display: flex; align-items: center; padding: 8px 12px; border-radius: 4px; text-decoration: none; font-size: 14px; font-weight: 500; background: transparent; transition: all 0.2s ease; border: 1px solid transparent; cursor: pointer; }
            .tdd-button:hover { background: rgba(167, 182, 194, 0.15); border-color: rgba(167, 182, 194, 0.3); transform: translateX(2px); }
            .tdd-button .tdd-icon { margin-right: 10px; font-size: 16px; width: 20px; text-align: center; }
            .tdd-button.todo .tdd-icon { color: #4A90E2; }
            .tdd-button.done .tdd-icon { color: #7ED321; }
    ...
    const init = () => {
        ...
        const observer = new MutationObserver(() => {
            if (!document.getElementById('tdd-section')) {
                addTDDSection();
            }
        });
